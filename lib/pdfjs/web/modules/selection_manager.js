            ...
            for (const model of models) {
                try {
                    ...
                    if (response.status === 429 || response.status === 404) {
                        console.log(`Synonyms: Model ${model} unavailable, trying next...`);
                        continue;
                    }
                    ...
                    if (data.error) {
                        console.log(`Synonyms: Model ${model} error:`, data.error.message);
                        continue;
                    }
                    ...
                    try {
                        ...
                        if (Array.isArray(parsed) && parsed.length > 0) {
                            console.log(`Successfully fetched synonyms using ${model}`);
                            resolve(parsed);
                            return;
                        }
                    } catch (e) {
                    ...
                    console.log(`Synonyms: Model ${model} failed:`, error.message);
                    continue;
                }
            }
    ...
    if (selection && selection.rangeCount > 0) {
        ...
        if (pageElement) {
            ...
            if (storedRects.length === 0 && allRects.length > 0) {
                console.log('SelectionManager: All rects filtered out, using fallback');
            ...
            console.log('SelectionManager: Stored', storedRects.length, 'rects for highlighting on page', storedPageNumber);
        }
    }
    ...
    chrome.storage.local.get(['noteFolders', 'noteSettings'], (result) => {
        ...
        dialog.querySelector('#saveNoteBtn').onclick = async () => {
            ...
            if (window.createHighlightFromRects && storedRects.length > 0) {
                try {
                    ...
                    if (highlight) {
                        ...
                        console.log('SelectionManager: Created highlight from stored rects:', highlightId);
                    }
                } catch (e) {
                ...
                try {
                    ...
                    if (highlight) {
                        highlightId = highlight.id;
                        console.log('SelectionManager: Created highlight from range:', highlightId);
                    }
                } catch (e) {
            ...
            if (window.globalNotesManager) {
                ...
                console.log('SelectionManager: Note saved to GlobalNotesManager', { noteId: note.id, folderId: folderId });
