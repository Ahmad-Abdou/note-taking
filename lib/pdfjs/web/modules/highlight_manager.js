...
window.updateHighlightColor = function (highlightId, newColor) {
    ...
    elements.forEach(el => {
        el.style.backgroundColor = hexToRgba(newColor, 0.90);
    });
...
function createHighlight(source, type, comment = null, color = null, options = {}) {
    ...
    if (mergedRects.length === 0) {
        console.log('No valid rects after filtering/merging');
        return null;
    }
...
function createHighlightFromRects(text, rects, pageNumber, type = 'highlight', comment = null, color = null, options = {}) {
    ...
    if (!pageElement) {
        console.log('createHighlightFromRects: Page element not found for page', pageNumber);
        return null;
    }
    ...
    if (mergedRects.length === 0) {
        console.log('No valid rects after filtering/merging (from rects)');
        return null;
    }
...
function renderHighlight(highlight, isNew = false) {
    ...
    // Skip rendering if page dimensions are invalid (page not fully rendered)
    if (pageWidth <= 0 || pageHeight <= 0) return;

    // Calculate border from page element styles for proper alignment
    const pageStyle = window.getComputedStyle(pageElement);
    ...
    highlight.rects.forEach((rect, index) => {
        ...
        if (rect.leftPercent !== undefined) {
            ...
            // Apply border compensation to align highlights with text
            highlightEl.style.left = `${left + borderLeft}px`;
            highlightEl.style.top = `${top + borderTop}px`;
    ...
    chrome.storage.local.get(['noteFolders', 'noteSettings', 'globalNotes'], (result) => {
        ...
        dialog.querySelector('.note-dialog-save').addEventListener('click', async () => {
            ...
            document.querySelectorAll(`[data-highlight-id="${highlight.id}"]`).forEach(el => {
                el.style.backgroundColor = hexToRgba(selectedColor, 0.90);
            });
...
function deleteHighlight(highlightId) {
    ...
    chrome.storage.local.get(['globalNotes'], (result) => {
        ...
        if (noteIndex !== -1) {
            ...
            console.log('Deleted associated note for highlight:', highlightId);
        }
    });
...
function initHighlightSystem() {
    ...
    console.log('Highlight system initialized with URL:', url);

    // Wait a bit for pages to render, then load highlights
    setTimeout(() => {
        loadHighlights();
    }, 500);
        ...
        buttons.forEach(btn => {
            if (btn.style.visibility === 'visible' && btn.style.opacity === '1') {
                ...
                if (mouseX >= rect.left && mouseX <= rect.right &&
                    ...
                    console.log('[Highlight] Delete button clicked via document handler');
