...
function _createRainSound(ctx) {
    ...
    lfo1.type = 'sine';
    lfo1.frequency.value = 0.1; // Slow swell
    
    const lfo2 = ctx.createOscillator();
    lfo2.type = 'triangle';
    lfo2.frequency.value = 0.3; // Faster gusting

    const lfoGain = ctx.createGain();
    lfoGain.gain.value = 0.05; // Modulation depth

    const gain = ctx.createGain();
    gain.gain.value = FocusState.soundVolume * 0.4; // Boost base volume since filters cut a lot

    // Signal flow: Source -> Filters -> HP/LP -> Gain
...
function _createOceanSound(ctx) {
    ...
    lpFilter1.frequency.value = 800; // Deep cutoff
    lpFilter1.Q.value = 0.2;

    const hpFilter = ctx.createBiquadFilter();
    hpFilter.type = 'highpass';
    hpFilter.frequency.value = 100; // Cut extreme sub-bass

    // Create 3 layered LFOs to simulate natural chaotic waves
    const lfo1 = ctx.createOscillator(); // Main wave
    lfo1.type = 'sine';
    lfo1.frequency.value = 0.05; // 20s cycle
    
    const lfo2 = ctx.createOscillator(); // Secondary wave
    lfo2.type = 'triangle';
    lfo2.frequency.value = 0.08; // 12s cycle
    
    const lfo3 = ctx.createOscillator(); // Surface wash
    lfo3.type = 'sine';
    lfo3.frequency.value = 0.15; // 6s cycle
    ...
    filterLfo.gain.value = 400; // Modulates filter cutoff freq for "crashing" brightness
    ...
    // Route volume modulation
    lfo1.connect(lfoGain1);
    lfo2.connect(lfoGain2);
    lfoGain1.connect(gain.gain);
    lfoGain2.connect(gain.gain);
    
    // Route frequency modulation (brightness increases as wave volume increases)
