...
async function processImportedData(text, format, settings = {}) {
    ...
    for (const eventData of events) {
        ...
        // Derive a stable ID from the calendar + the ICS UID (if present).
        // Falling back to a content hash ensures uniqueness even for non-ICS sources.
        let stableId;
        if (eventData.icsUid) {
        ...
        await ProductivityData.DataStore.saveScheduleEvent(event);
        // Replace in ScheduleState if already present, otherwise push
